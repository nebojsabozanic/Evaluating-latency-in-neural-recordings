...
%     Tau = [-T+0.01 : .01 : T-0.01];
    ...
    T_d = zeros(nStats, numel(Tau));
    Tc_d = zeros(nStats, numel(Tau));
    Tv_d = zeros(nStats, numel(Tau));
...
%                 referentTrain = T.*referentTrain./max(referentTrain);
        ...
        for count = 1 : numel(Tau)
          ...
          %   clc 
            ...
            vc = SPIKY_Victor_MEX(spikes{1}, spikes{2}, 0.001); %put as an argument 100
    ...
    normT_d = (T_d - mean(T_d, 1))./std(T_d, [], 1);
    normTc_d = -(Tc_d - mean(Tc_d, 1))./std(Tc_d, [], 1);
    normTv_d = (Tv_d - mean(Tv_d, 1))./std(Tv_d, [], 1);
